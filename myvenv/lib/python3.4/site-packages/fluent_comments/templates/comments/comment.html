...
{% endcomment %}
{% load i18n %}
        <div style="border:1px solid black; border-radius: 3px;padding: 15px; background-color: #E6E6FA;" {% if preview %} id="comment-preview"{% else %} id="c{{ comment.id }}"{% endif %} class="comment-item">
          {% if preview %}<h4><b>{% trans "Preview of your comment" %}</b></h4>{% endif %}
            ...
            <div class="comment-text" style="color: black;">{{ comment.comment|linebreaks }}</div>
